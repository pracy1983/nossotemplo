// Script simples para testar o envio de email (formato CommonJS)
...
async function testEmail() {
  ...
  if (!process.env.EMAIL_USER || !process.env.EMAIL_PASS) {
    console.error('\n\u274C Erro: Credenciais de email não estão definidas no arquivo .env');
    return;
  }
  ...
  try {
    ...
    const transporter = nodemailer.createTransport({
      ...
      tls: {
        // Não verificar certificado para fins de teste
