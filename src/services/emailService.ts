...
export const sendPasswordResetEmail = async (to: string, resetLink: string, name: string): Promise<boolean> => {
  console.log('Reset Link recebido em emailService:', resetLink);
...
export const sendInviteEmail = async (to: string, inviteLink: string, name: string, tempPassword?: string): Promise<boolean> => {
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      ${tempPassword ? `
      <p><strong>Sua senha temporária:</strong> <code>${tempPassword}</code></p>
      <p>Use esta senha para fazer login após aceitar o convite. Recomendamos que você altere esta senha após o primeiro acesso.</p>
      ` : ''}
      <p>Atenciosamente,<br>Equipe Nosso Templo</p>
    </div>
  `;
